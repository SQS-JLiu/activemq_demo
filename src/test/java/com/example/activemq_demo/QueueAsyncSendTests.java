...
@SpringBootTest
class QueueAsyncSendTests {

	private static String url = "tcp://127.0.0.1:61616";
	private static String queueName = "queue_asyncSend";

	@Test
	void producerTest() throws JMSException {
		//创建连接工厂
		ActiveMQConnectionFactory factory = new ActiveMQConnectionFactory(url);
		factory.setUseAsyncSend(true);//使用异步发送
		//创建连接
		...
		for (int i = 0; i < 10; i++) {
			...
			textMessage.setJMSCorrelationID(UUID.randomUUID().toString()+"$"+"id");
